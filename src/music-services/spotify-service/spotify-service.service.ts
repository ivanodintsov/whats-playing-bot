...
export class SpotifyService extends MusicServiceCoreService {
  ...
  private async _searchTracks(
    ...
    const tracks = response.body.tracks.items.map(this._createTrackEntity);
  ...
  async getCurrentTrack() {
    ...
    return this._createTrackEntity(response.body.item);
  }
  ...
  async getTrack({ id }: { id: any }) {
    ...
    return this._createTrackEntity(response.body);
  }
  ...
  async getArtistAlbums({
    ...
    return {
      items: body.items.map(this._createAlbumSimple),
      pagination: {
        next: body.next,
  ...
  async getAlbumTracks({
    ...
    return {
      items: body.items.map(this._createTrackSimple),
      pagination: {
        next: body.next,
        offset: body.offset.toString(),
      },
  ...
  async toggleFavorite({ uris }: { uris: [SpotifyURI] }) {
    const toggle = async () => {
      ...
      if (isContains) {
        const response = await this.api.removeFromMySavedTracks(trackIds);
        return { response: { ...response }, action: 'removed' };
      }

      const response = await this.api.addToMySavedTracks(trackIds);
  ...
  private _createTrackEntity(item: SpotifyItem): TrackEntity {
  ...
  private _createTrack(track: SpotifyApi.TrackObjectFull): ITrack {
    return {
      ...this._createTrackSimple(track),
      artists: track.artists.map(this._createArtistSimple),
  ...
  private _createAlbumSimple(
    album: SpotifyApi.AlbumObjectSimplified,
  ): IAlbumSimple {
    const artists: IArtistSimple[] = [];
